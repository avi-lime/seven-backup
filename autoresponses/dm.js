...
client.on('message', message => {
    ...
    if (commandName === 'dm' || commandName === 'reply') {
        ...
        if (toSend) {
            toSend.send(message.content.replace(prefix + commandName + ' ' + args[0], '')).catch(message.channel.send(`can't send a dm to this user`));
            message.delete;
            ...
            if (user) {
                user.send(message.content.replace(prefix + commandName + ' ' + args[0], '')).catch(message.channel.send(`can't send a dm to this user`));
                message.delete;
    ...
    if (commandName === 'dmw' || commandName === 'replyw') {
        ...
        const toSend = message.mentions.members.first();
        if (toSend) {
            toSend.send(`${message.author.tag}: ${message.content.replace(prefix + commandName + ' ' + args[0], '')}`).catch(message.channel.send(`can't send a dm to this user`));
            message.delete;
            message.channel.send(`message sent to ${toSend.displayName}`);
        } else if (message.mentions.users.size === 0) {
            const userid = args[0].toString();
            const user = message.guild.members.cache.get(userid);
            if (user) {
                user.send(`${message.author.tag}: ${message.content.replace(prefix + commandName + ' ' + args[0], '')}`).catch(message.channel.send(`can't send a dm to this user`));
                message.delete;
                message.channel.send(`message sent to ${user.displayName}`);
            } else return message.channel.send(`No user found, either mention a proper user or use their ID`);
        }
